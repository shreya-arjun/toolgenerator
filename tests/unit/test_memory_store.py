...
def _run_common_memory_tests(store):
    ...
    store.add(token_session_1, scope="session", metadata={"conversation_id": "conv1"})
    results = store.search(token_session_1, scope="session", conversation_id="conv1")
    assert len(results) >= 1
    assert any(token_session_1 in (r.get("content") or "") for r in results)

    store.add(token_session_2, scope="session", metadata={"conversation_id": "conv2"})
    conv2_results = store.search(token_session_1, scope="session", conversation_id="conv2")
    ...
    session_results = store.search(token_corpus, scope="session", conversation_id="conv1")
...
def test_fake_memory_add_then_search_returns_entry():
    ...
    store.add("hello-memory", scope="session", metadata={"conversation_id": "conv1"})
    results = store.search("hello-memory", scope="session", conversation_id="conv1")
    assert len(results) >= 1
...
def test_fake_memory_session_conv_isolation():
    store = FakeMemoryStore()
    store.add("conv1-only", scope="session", metadata={"conversation_id": "conv1"})
    results = store.search("conv1-only", scope="session", conversation_id="conv2")
    assert all("conv1-only" not in r["content"] for r in results)


def test_fake_memory_session_not_returned_in_corpus():
    store = FakeMemoryStore()
    store.add("session-only", scope="session", metadata={"conversation_id": "conv1"})
...
def test_fake_memory_corpus_not_returned_in_session():
    ...
    results = store.search("corpus-only", scope="session", conversation_id="conv1")
...
def test_mem0_memory_add_then_search_returns_entry():
    ...
    store.add(token, scope="session", metadata={"conversation_id": "conv1"})
    results = store.search(token, scope="session", conversation_id="conv1")
    assert len(results) >= 1
...
def test_mem0_memory_session_conv_isolation():
    ...
    store.add(token, scope="session", metadata={"conversation_id": "conv1"})
    results = store.search(token, scope="session", conversation_id="conv2")
